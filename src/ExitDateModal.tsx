...
// Ekip Yönetimi satırındaki hızlı ikon ve Personel Kartı'ndaki "Şirketten
// Çıkar" butonu tarafından paylaşılan tek modal. Personel bugün değil,
// örneğin bir ay önce zaten ayrılmış olabilir — bu yüzden çıkış tarihi
// seçilebilir olmalı (varsayılan bugün).
export default function ExitDateModal({ memberName, loading, onConfirm, onCancel }: Props) {
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm border border-slate-100 dark:border-slate-700 animate-scaleIn">
        <div className="p-5 space-y-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-bold text-gray-800 dark:text-white">Şirketten Çıkar</h3>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                <span className="font-bold">{memberName}</span> adlı personel şirketten çıkarılacak.
              </p>
            </div>
          </div>

          <div>
            <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase">Ayrılış Tarihi</label>
            <input
              type="date"
              value={exitDate}
              max={today}
            ...
            <p className="text-[10px] text-slate-400 mt-1">
              Personel daha önce ayrılmışsa gerçek tarihi seçin — geçmiş aylara ait otomatik maaş giderleri buna göre düzeltilir.
            </p>
          </div>
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            >
              <LogOut size={14} /> {loading ? 'Çıkarılıyor...' : 'Çıkar'}
            </button>
          </div>
        </div>
      </div>
