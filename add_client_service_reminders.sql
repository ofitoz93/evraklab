-- ==========================================================
-- ADD SERVICE START DATE AND CONTRACT FILE TO CLIENTS
-- & SETUP CONTRACT REMINDER MAIL NOTIFICATIONS
-- ==========================================================

-- 1. Add columns to consultant_clients table
ALTER TABLE public.consultant_clients
ADD COLUMN IF NOT EXISTS service_start_date DATE,
ADD COLUMN IF NOT EXISTS contract_file_url TEXT;

-- 2. Create PL/pgSQL send_client_contract_reminders function
...
BEGIN
  ...
  FOR rec IN
    ...
    WHERE p.role IN ('premium_corporate', 'corporate_chief')
      ...
      -- Expiring in <= 10 days or already expired (1 year contract duration)
      AND ((cc.service_start_date + INTERVAL '1 year')::date - CURRENT_DATE) <= 10
  LOOP
    r_rows := '';
    FOR client_rec IN
      SELECT 
        ...
        (cc.service_start_date + INTERVAL '1 year')::date AS expiry_date,
        ((cc.service_start_date + INTERVAL '1 year')::date - CURRENT_DATE) AS days_remaining
      FROM public.consultant_clients cc
      WHERE cc.consultant_company_id = rec.org_id
        AND cc.service_start_date IS NOT NULL
        AND ((cc.service_start_date + INTERVAL '1 year')::date - CURRENT_DATE) <= 10
      ORDER BY ((cc.service_start_date + INTERVAL '1 year')::date - CURRENT_DATE) ASC
    LOOP
    ...
    r_html := '<div style="background-color: #f1f5f9; padding: 40px 16px; font-family: -apple-system, BlinkMacSystemFont, ''Segoe UI'', Roboto, ''Helvetica Neue'', Arial, sans-serif;">' ||
              ...
              '<p style="font-size: 14px; color: #475569; line-height: 1.6; margin-bottom: 24px;">Hizmet verdiğiniz aşağıdaki işletmelerin 1 yıllık sözleşme sürelerinin dolmasına 10 günden az kalmış veya süreleri dolmuştur. İşletmelerin hizmet sürelerinin kesintiye uğramaması için gerekli sözleşme yenileme işlemlerini yapmanızı rica ederiz.</p>' ||
...
-- 3. Schedule daily cron job (runs every day at 07:00 UTC, which is 10:00 Turkey Local Time)
-- NOTE: Requires pg_cron extension to be enabled in Supabase.
CREATE EXTENSION IF NOT EXISTS pg_cron;
-- Unschedule if exists to avoid duplicate schedules
SELECT cron.unschedule(jobid) FROM cron.job WHERE jobname = 'send-client-contract-reminders-daily';
SELECT cron.schedule('send-client-contract-reminders-daily', '0 7 * * *', 'SELECT public.send_client_contract_reminders();');
